...
def read_img_and_convert_to_binary(filename):
    ...
    # filename = filename.replace("\\", "\")  # Convert Windows-style paths to UNIX-style
    original_img = cv2.imread(filename)

    print(original_img)
    if original_img and os.path.exists(filename):
        original_img = cv2.resize(original_img, (int(original_img.shape[1] / SCALSIZE), int(original_img.shape[0] / SCALSIZE)), interpolation=cv2.INTER_AREA)
